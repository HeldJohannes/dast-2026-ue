title = 'Check for reused dataset declaration'
description = (
...
def meta(test_url_base: str) -> dict:
    return {
        ...
        "dcat:endpointURL": {"@id": f"{test_url_base}/1"},
        "dcat:version": {"@language": "en", "@value": "1.0.0"},
        "dcat:keyword": [
            {"@language": "en", "@value": "reused dataset"},
            {"@language": "en-US", "@value": "data reuse"},
...
def assess(madmp: dict) -> dict:
    ...
    for ds in datasets:
        ...
        # Check distributions inside dataset (under "distribution" or "rdap:distribution")
    ...
    if found:
        return {"outcome": "pass", "log": "Field is_reused found on at least one dataset or distribution."}
    return {"outcome": "fail", "log": "No dataset or distribution entry declares the is_reused field."}
